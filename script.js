...
const SyncManager = {
    ...
    async flush() {
        ...
        try {
            ...
            console.log(`☁️ [SYNC] Успішно збережено в хмару: ${templates.length} шаблонів`);
            markSyncedNow();
        } catch (error) {
            console.error("❌ [SYNC ERROR] Помилка запису в Firebase:", error);
...
function clearAllTemplates() {
    ...
    if (confirm('УВАГА: Видалити ВСІ шаблони?')) {
        ...
        saveTemplates(); 
        checkEmptyTemplatesState();
        showNotification("Усі шаблони видалено.", 'success');
    }
}
...
fieldGroup.dataset.onuMode             = onuMode || (switchMode ? 'SWITCH' : 'REG');
    ...
    copyButton.className = 'copy-template-btn';
            copyButton.onclick = () => {
        // === ЗМІНЕНО: кнопка працює тільки з головним textarea шаблону ===
        const target = fieldGroup.querySelector('textarea');
        if (!target) return;
...
let lastConfirmedOltName = null;
   ...
   configPanel.innerHTML = `
    ...
    <input type="text" class="config-login-input has-emoji" placeholder="Login, ПІБ або назва компанії" title="Введіть згенерований логін або ПІБ українською" autocomplete="off">
    ...
    <span class="input-emoji">🔎</span>
    <input type="text" class="config-olt-select has-emoji" placeholder="Пошук OLT..." autocomplete="off" title="Почніть вводити назву OLT">
    ...
    <div class="olt-dropdown-list"></div>
            ...
            <div class="config-row">
                <div class="config-field config-field--sn">
    <span class="input-emoji">🏷️</span>
    <input type="text" class="config-sn-input has-emoji" placeholder="SN / MAC" title="Серійний номер або MAC" autocomplete="off">
</div>
<div class="config-field config-field--port">
    <span class="input-emoji">🔌</span>
...
// =========================================================================
// === БЕЗЛІМІТНИЙ ДЕТЕКТОР ОНОВЛЕНЬ (ЧЕРЕЗ HEADERS СЕРВЕРА) ===
// =========================================================================
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Якщо сайт відкрито як локальний файл на диску — перевірка не потрібна
    if (window.location.protocol === 'file:') {
        dot.className = 'site-status-dot status-green';
        statusBtn.title = 'Локальний режим (файл на ПК)';
        return;
    }
    ...
    async function checkServerVersion() {
        ...
        try {
            ...
            console.warn("Перевірка версії:", err);
            scheduleNextCheck(30000);
        }
    }
